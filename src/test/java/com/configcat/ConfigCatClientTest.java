...
class ConfigCatClientTest {
    ...
    void ensuresApiKeyIsNotNull() {
        ...
        assertEquals("SDK Key cannot be null or empty.", exception.getMessage());
    }
    ...
    void testSDKKeyValidation() throws IOException {
        ...
        ConfigCatClient clientLocalOnly = ConfigCatClient.get("sdk-key-90123456789012", options -> {
            options.flagOverrides(OverrideDataSource.map(new HashMap<>()), OverrideBehaviour.LOCAL_ONLY);
        });
    ...
    void getAllValueDetails() throws IOException {
        ...
        List<EvaluationDetails<?>> allValuesDetails = cl.getAllValueDetails();
    ...
    void getAllValueDetailsAsync() throws IOException, ExecutionException, InterruptedException {
        ...
        List<EvaluationDetails<?>> allValuesDetails = cl.getAllValueDetailsAsync().get();
